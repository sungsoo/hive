...
public class DecimalColAddDecimalColumn extends VectorExpression {
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector1.isRepeating && inputColVector2.isRepeating) {
      DecimalUtil.addChecked(0, vector1[0], vector2[0], outputColVector);
    } else if (inputColVector1.isRepeating) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          DecimalUtil.addChecked(i, vector1[0], vector2[i], outputColVector);
        }
      } else {
        for(int i = 0; i != n; i++) {
          DecimalUtil.addChecked(i, vector1[0], vector2[i], outputColVector);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          DecimalUtil.addChecked(i, vector1[i], vector2[0], outputColVector);
        }
      } else {
        for(int i = 0; i != n; i++) {
          DecimalUtil.addChecked(i, vector1[i], vector2[0], outputColVector);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          DecimalUtil.addChecked(i, vector1[i], vector2[i], outputColVector);
        }
      } else {
        for(int i = 0; i != n; i++) {
          DecimalUtil.addChecked(i, vector1[i], vector2[i], outputColVector);
