...
public class FilterDecimalColumnNotBetween extends VectorExpression {
  ...
  public FilterDecimalColumnNotBetween(int colNum, Decimal128 leftValue, Decimal128 rightValue) {
    this.colNum = colNum;
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.noNulls) {
      if (inputColVector.isRepeating) {
        ...
        if (!(vector[0].compareTo(leftValue) < 0 || vector[0].compareTo(rightValue) > 0)) {
        ...
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          if (!(leftValue.compareTo(vector[i]) <= 0 && vector[i].compareTo(rightValue) <= 0)) {
            sel[newSize++] = i;
          }
        }
        batch.size = newSize;
